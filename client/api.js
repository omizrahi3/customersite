...
export default {
  ...
  checkout: {
    existing: data => {
      const instance = axios.create({ timeout: 3000 });
      instance.defaults.headers.common['token'] = data.Token;
      return axios.post("http://www.qa.getchatwith.com/api/CreateWebTransactionExisting", data.checkout).then(res => res.data.Response.Response)
    },
    new: data => {
      const instance = axios.create({ timeout: 3000 });
      instance.defaults.headers.common['token'] = data.Token;
      return axios.post("http://www.qa.getchatwith.com/api/CreateWebTransactionNew", data.checkout).then(res => res.data.Response.Response)
    },
    update: data => {
      const instance = axios.create({ timeout: 3000 });
      instance.defaults.headers.common['token'] = data.Token;
